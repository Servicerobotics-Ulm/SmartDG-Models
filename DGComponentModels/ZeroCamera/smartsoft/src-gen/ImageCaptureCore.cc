...
ImageCaptureCore::ImageCaptureCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
void ImageCaptureCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const ImageCapture* imageCapture = dynamic_cast<const ImageCapture*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(imageCapture);
		}
	}
}

void ImageCaptureCore::attach_interaction_observer(ImageCaptureObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void ImageCaptureCore::detach_interaction_observer(ImageCaptureObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int ImageCaptureCore::execute_protected_region()
{
...
void ImageCaptureCore::updateAllCommObjects()
{
...
void ImageCaptureCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int ImageCaptureCore::getPreviousCommObjId()
{
